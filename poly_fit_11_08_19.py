    ...
    for plateifu in gal_list:
        ...
        try:
            ...
            if (np.max(rad) >= 1.5) & ((len(n2o2_good) >= 100) & (len(n2s2_good) >= 100) & (len(pp04_good) >= 100)):
                ...
                # 'vary = False' fixes the parameter. Setting min=1 and max=2 will allow that parameter to vary between 1 and 2.
                ...
                #def gas_regulator_model_residual(params,data):
                    #define the residuals e.g residuals =data - (m*x + b)
                    #return residuals
...
def main_4():
    ...
    x = np.linspace(0.25, 2.25, 500) # Fake x data with evenly spaced numbers over a specified interval of 0 to 3 (Re)
    y = 8.8 + np.where(x<1.2, -0.1*x, np.where(x<2.5, -1.3*x, -1.3*x))  # Fake linear metallicity
