    ...
    for plateifu in gal_list:
        ...
        try:
            ...
            if (np.max(rad) >= 1.5) & ((len(n2o2_good) >= 100) & (len(n2s2_good) >= 100) & (len(pp04_good) >= 100)):
                ...
                #  Performs least squares polynomial fit on 2nd degree poly
                poly_n2o2 = np.polyfit(x, y_n2o2, 3) # Returns polynomial coefficients, highest power first
                xmodel = np.linspace(low, 2.5, len(y_n2o2))
                ymodel_n2o2 = np.polyval(poly_n2o2, xmodel) # Evaluates polynomial at specific values
                der_n2o2 = np.absolute(np.polyder(poly_n2o2, 1)) # Returns first order derivative of function
                ...
                if (((.3 <= r01_n2o2 < 1.) | (.3 <= r10_n2o2 < 1.) | (.3 <= r12_n2o2 < 1.) | (.3 <= r21_n2o2 < 1.)) & (der_n2o2.any() >=0.3)):
                ...
                der_n2s2 = np.absolute(np.polyder(poly_n2s2, 1)) # Returns first order derivative of function
                ...
                # Will print out statements if galaxy's oxygen abundance gradient has a slope ratio greater than 30%
                if (((.3 <= r01_n2s2 < 1.) | (.3 <= r10_n2s2 < 1.) | (.3 <= r12_n2s2 < 1.) | (.3 <= r21_n2s2 < 1.)) & (der_n2s2.any() >=0.3)):
                ...
                #  Performs least squares polynomial fit on 2nd degree poly
                poly_pp04 = np.polyfit(x, y_pp04, 3) # Returns polynomial coefficients, highest power first
                ymodel_pp04 = np.polyval(poly_pp04, xmodel) # Evaluates polynomial at specific values
                der_pp04 = np.absolute(np.polyder(poly_pp04, 1)) # Returns first order derivative of function
                ...
                # Will print out statements if galaxy's oxygen abundance gradient has a slope ratio greater than 30%
                if (((.3 <= r01_pp04 < 1.) | (.3 <= r10_pp04 < 1.) | (.3 <= r12_pp04 < 1.) | (.3 <= r21_pp04 < 1.)) & (der_pp04.any() >=0.3)):
                ...
                plt.plot(xvals, yinterp_n2o2, '-*', c='red', lw=2, zorder=2, ms=3, label='Interp.') # plots points found for calculating the slope change
                plt.plot(xmodel, ymodel_n2o2, c='blue', zorder=3, label='Poly Fit')
                ...
                plt.plot(xvals, yinterp_n2s2, '-*', c='red', lw=2, zorder=2, ms=3, label='Interp.') # plots points found for calculating the slope change
                ...
                #PP04
                plt.subplot(3,1,3)
                plt.errorbar(rad.ravel(), pp04_met.ravel(),yerr=pp04_met_err.ravel(),fmt='o',ecolor='plum', mfc='indigo',mec='indigo',ms=1, label='PP04', zorder=1) # plots metallicity points with error bars
                plt.plot(xvals, yinterp_pp04, '-*', c='red', lw=2, zorder=2, ms=3, label='Interp.') # plots points found for calculating the slope change
                plt.plot(xmodel, ymodel_pp04, c='blue', zorder=3, label='Poly Fit')
                plt.axvline(x = 0.5, color='deeppink', lw=1, zorder=10) # plots vertical line at .5 effective radii
                plt.axvline(x = 2, color='deeppink', lw=1, zorder=10) # plots vertical line at 2 effective radii
                plt.xlabel('R/$R_e$')
                plt.ylabel('12+log(O/H)')
                ...
                #plt.savefig('/Users/jalynkrause/Documents/astro/grad_ratio_1/plots0820/' + plateifu + '.png')
                plt.show()
                plt.close('all')
