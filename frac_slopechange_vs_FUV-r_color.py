...
# fraction of galaxies where the metallicity profile changes slopes vs. FUV - r color for fits file v0
def fits_v0():
    hdu_v0 = fits.open('zooinverse_summary_v0.fit')
    plateifu = hdu_v0[1].data['PLATEIFU']
    log_mass = hdu_v0[1].data['LOG_MASS']
    slope_change = hdu_v0[1].data['SLOPE_CHANGE'] # fraction of galaxies with slope changes as a function of stellar mass
    FUV_r = hdu_v0[1].data['FNUGRIZ_ABSMAG'] # array of 7 colors in this order:  FUV, NUV, u, g, r, i, z
    data = hdu_v0[1].data
    ...
    for i in range(0, len(FUV_r) -1):
        x = i 

    # plotted figure of fraction of galaxies with slope changes & stellar mass
    plt.plot(x, frac, label='Summary v0 Fits File', linewidth = 3)
    plt.scatter(x, frac, lw = 3)
...
def fits_v2():
    ...
    FUV_r = hdu_v2[1].data['FNUGRIZ_ABSMAG'] # array of 7 colors in this order:  FUV, NUV, u, g, r, i, z
    data = hdu_v2[1].data
    ...
    # plotted figure of fraction of galaxies with slope changes & stellar mass
    plt.plot(x, frac, label='Summary v2 Fits File', linewidth = 3)
    plt.scatter(x, frac, lw = 3)
